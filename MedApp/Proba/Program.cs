...
namespace Proba
{
    class Program
    {
        ...
        public static void SignPdf()
        {
            ...
            string certificate = "MIIIPzCCBiegAwIBAgIQV" +
            ...
            string signature = "iEo1%2FrYXLx4%2FZKYJE4pHA" +
            ...
            var sign = signature.Replace("%2F", "/").Replace("%2B", "+").Replace("%3D", "=");
            byte[] signatureBytes = Convert.FromBase64String(sign);

            


            var cert = certificate.Replace("%2F", "/").Replace("%2B", "+");
            byte[] certificateBytes = Convert.FromBase64String(cert);



            //byte[] signatureBytess = Encoding.UTF8.GetBytes(signature);
            //byte[] certificateBytess = Encoding.UTF8.GetBytes(certificate);
            ...
            using (PdfReader reader = new PdfReader(@"C:\\Users\CSVarazdin\Documents\GitHub\TimeRedistribution\MedApp\SecretToken\prep.pdf"))
            using (PdfDocument document = new PdfDocument(reader))
            using (FileStream fout = new FileStream(@"C:\\Users\CSVarazdin\Documents\GitHub\TimeRedistribution\MedApp\SecretToken\signed.pdf", FileMode.Create))
            {
